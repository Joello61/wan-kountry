// JoelTech Service Worker - Optimisé SEO 2025
// Version: 2.0.0 - Compatible avec les dernières mises à jour Google

const CACHE_NAME = "joeltech-cache-v2.0.0"
const CACHE_STATIC_NAME = "joeltech-static-v2.0.0"
const CACHE_DYNAMIC_NAME = "joeltech-dynamic-v2.0.0"
...
async function networkFirstStrategy(request) {
  try {
    ...
    if (request.headers.get("accept")?.includes("text/html")) {
      ...
      return offlineResponse || new Response("Page hors ligne non disponible", {
        status: 503,
...
self.addEventListener("push", (event) => {
  if (event.data) {
    ...
    event.waitUntil(
      self.registration.showNotification("JoelTech", options)
    )
  }
})
